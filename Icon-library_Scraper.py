...
def scrape(url):
    ...
    elements = container_soup.find_all("img", src=re.compile(".jpg"))

    for i, element in enumerate(elements):
        print(f'Download {i + 1} / {len(elements)}')
        gif_url = element['src']
        if not str(gif_url).startswith("https"):
            gif_url = 'https://icon-library.com' + str(gif_url)
...
if __name__ == '__main__':
    ...
    base_url = 'https://icon-library.com/icon/animated-icon-gifs-0.html'
    # categories = ['other']
    scrape(base_url)
    print("Done")
